...
class EntityService:
    ...
    async def entity_exists(self, entity_id: str) -> bool:
        """Distinguishes 'entity not found' from 'entity with no
        transactions' — export_subgraph always returns at least the
        queried node, so it can't be used for this check on its own."""
        entity_hash = self.hash_entity_id(entity_id)
        async with self.driver.session() as session:
            result = await session.run(
                "MATCH (e:Entity {id: $id}) RETURN e.id AS id LIMIT 1", id=entity_hash
            )
