...
pub struct ComplianceOracle {
    verdicts: Mapping<String, VerdictRecord>,
...
impl ComplianceOracle {
    pub fn init(&mut self, mint_authority: Address, agent_key: Address) {
        self.mint_authority.set(mint_authority);
        self.agent_key.set(agent_key);
        self.policy_version.set(1u64);
    }
    ...
    pub fn record_verdict(
        ...
        let expected = self.mint_authority.get().unwrap();
        if caller != expected {
            self.env().revert(OdraError::user(1));
        }

        let current_version = self.policy_version.get().unwrap_or(1u64);
        ...
        let agent_key = self.agent_key.get().unwrap();
